import { skechersProducts } from "./men7";
...
const Meni6 = () => {
  ...
  const product = skechersProducts.find(p => p.id === Number(id));
  ...
  return (
    <>
      <div className="w-300 shadow-lg rounded-lg bg-white  overflow-hidden p-2 ml-40 mt-10  transition-shadow duration-300 ">
        <div className="flex rounded-2xl   justify-center bg-yellow-50 items-center">
          <div className="cart flex ">
            <div className="w-1/2 h-full p-5">
              <img src={product.image} alt="no" className="w-full h-150 object-contain rounded-3xl" />
            </div>
            <div className="w-1/2 pt-10">
              <h1 className="font-bold text-4xl">{product.brand} </h1>

              <p className="font-semibold text-2xl">{product.name}</p>
              <p className="w-40 pl-3 border-white bg-green-300 rounded-lg p-1 mt-5 ">{product.rating}</p>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {sizes.map((size) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-full border font-medium ${selectSize === size
              ...
              <div className="mt-5">
                ...
                <input
                  className="border rounded-lg p-1 ml-2 w-20 h-8"
                  type="number"
                  value={quantity}
              ...
              <div>
                <input className="border-2 ml-3 mt-3 pl-3 w-72 rounded-xl" type="number" placeholder="Enter your Pincode" />
                ...
                <div className="flex  gap-10 mt-5">
                  ...
                  <button 
                  onClick={handleClick}
                  className="flex items-center gap-3 border mt-5 rounded-2xl px-10 h-10 w-50 bg-emerald-200 hover:bg-emerald-500 transition-all text-sm">
